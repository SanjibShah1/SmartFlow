...
WINDOW_SIZE = 5  # Number of frames to use for speed calculation
PIXELS_TO_METERS = 0.05  # Example: 1 pixel = 0.05 meters (calibrate this)
...
if cap is not None and cap.isOpened():
    ...
    while cap.isOpened():
        ...
        for box in results[0].boxes:
            ...
            if len(vehicle['positions']) > WINDOW_SIZE:
            ...
            if len(vehicle['positions']) >= 2:
                ...
                for i in range(1, len(vehicle['positions'])):
                    prev_x, prev_y = vehicle['positions'][i - 1]
                    curr_x, curr_y = vehicle['positions'][i]
                    distance_pixels = math.sqrt((curr_x - prev_x) ** 2 + (curr_y - prev_y) ** 2)
                ...
                # Calculate total time elapsed for the window
                total_time = frame_time * (len(vehicle['positions']) - 1)

                # Calculate average speed in meters per second
                average_speed_mps = total_distance_meters / total_time

                # Convert speed to kilometers per hour
                average_speed_kmh = average_speed_mps * 3.6
                vehicle['speed'] = average_speed_kmh

                # Check for overspeeding
                if average_speed_kmh > speed_limit:
                    vehicle['overspeeding'] = True
                    overspeeding_vehicles[track_id] = average_speed_kmh  # Store ID and speed
