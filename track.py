...
lane_outgoing = defaultdict(int)
lane_incoming = defaultdict(int)
counting_line_y = 350  # Adjust based on your video
num_lanes = 2  # Set this to match your video
    ...
    for r in results:
        ...
        if args.show or args.save:
            ...
            for box in r.boxes:
                ...
                # Determine lane
                lane_width = img.shape[1] / num_lanes
                current_lane = min(int(x_center // lane_width) + 1, num_lanes)
                vehicle['lane'] = current_lane
                       ...
                       (prev_y < counting_line_y and y_center >= counting_line_y):
                        if not vehicle['counted']:
                            if direction == "outgoing":
                                lane_outgoing[current_lane] += 1
                            else:
                                lane_incoming[current_lane] += 1
                            vehicle['counted'] = True

            # Draw counts on frame
            for lane in range(1, num_lanes + 1):
                text = f'Lane {lane}: Out {lane_outgoing[lane]} | In {lane_incoming[lane]}'
                cv2.putText(img, text, (10, 30 * lane),
